...
tab1, tab2= st.tabs(["📊 Dashboard", "🏏 Win Probability"])
...
with tab2:
    st.subheader("Live Win Probabiltiy Predictor")
    
    wp_model = joblib.load("win_prob_model.pkl")
    wp_df= pd.read_csv("win_probabilty_dataset.csv")

    seasons = sorted(wp_df['season'].dropna().unique())
    sel_seasons=st.selectbox("Select Season", seasons, keys="wp_season")
